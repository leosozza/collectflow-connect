...
import { fetchTiposDivida, upsertTipoDivida, deleteTipoDivida } from "@/services/cadastrosService";
...
const TipoDividaList = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: any) => upsertTipoDivida(data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["tipos_divida"] }); toast.success("Salvo!"); setDialogOpen(false); },
    onError: () => toast.error("Erro ao salvar"),
  });

  const deleteMutation = useMutation({
    mutationFn: deleteTipoDivida,
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["tipos_divida"] }); toast.success("Excluído!"); },
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editing ? "Editar Tipo de Dívida" : "Novo Tipo de Dívida"}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div><Label>Nome *</Label><Input value={nome} onChange={e => setNome(e.target.value)} placeholder="Ex: Boleto, Cartão..." /></div>
