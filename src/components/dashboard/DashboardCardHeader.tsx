...
const DashboardCardHeader = ({ icon: Icon, title, right, className }: Props) => {
  return (
    <div
      className={cn(
        "px-4 py-2.5 shrink-0 bg-secondary text-secondary-foreground flex items-center gap-2",
        className
      )}
    >
      <div className="rounded-lg p-1.5 inline-flex bg-primary/15 ring-1 ring-primary/30 shrink-0">
        <Icon className="w-3.5 h-3.5 text-primary" strokeWidth={2.5} />
      </div>
      <h2 className="text-[12px] font-semibold tracking-[0.04em] uppercase text-white/95 truncate">
        {title}
      </h2>
