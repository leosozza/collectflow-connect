...
import { fetchSerasaLogs, type SerasaLog } from "@/services/serasaService";
...
const SerasaLogsCard = ({ refreshKey }: Props) => {
  const { toast } = useToast();
  const [logs, setLogs] = useState<SerasaLog[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const data = await fetchSerasaLogs();
        setLogs(data);
      } catch (e: any) {
  ...
  const ACTION_LABELS: Record<string, string> = {
    negativar: "Negativação",
    batch_negativar: "Negativação em Lote",
    remover: "Remoção",
    auto_remover: "Remoção Automática",
