...
import { batchCreateProtestTitles } from "@/services/protestoService";
...
const ProtestoBatchDialog = ({ onCreated }: Props) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await batchCreateProtestTitles(titles, tenant.id, user.id);
      toast({ title: "Lote enviado", description: `${titles.length} título(s) registrado(s)` });
      setOpen(false);
      setPreview([]);
      onCreated();
    } catch (e: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Envio em Lote de Títulos a Protesto</DialogTitle>
          <DialogDescription>
            Filtre os devedores e envie múltiplos títulos de uma vez
          </DialogDescription>
        </DialogHeader>
        ...
        {preview.length > 0 && (
          <>
            ...
            <Button onClick={handleSend} disabled={sending} className="w-full gap-2">
              ...
              Enviar {preview.length} Título(s) a Protesto
            </Button>
          </>
        )}
      </DialogContent>
...
export default ProtestoBatchDialog;
