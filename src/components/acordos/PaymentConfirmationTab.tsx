    ...
    try {
      ...
      if (wasConfirmed) {
        ...
        if (agr && Math.abs(delta) > 0.001) {
          if (delta > 0) {
            await registerAgreementPayment(agr.client_cpf, agr.credor, delta, agr.tenant_id);
          } else {
            await reverseAgreementPayment(agr.client_cpf, agr.credor, Math.abs(delta), agr.tenant_id);
          }
        }
