...
export const cancelAgreement = async (id: string): Promise<void> => {
  try {
    ...
    // Toda a lógica (lock + soma das fontes + FIFO + idempotência + auditoria)
    // roda em uma única transação SQL: apply_agreement_credit_on_cancel.
    // Inclui manual_payments (confirmed/approved), portal_payments (paid)
    // e negociarie_cobrancas (pago). Crédito é abatimento, não receita.
    if (agreement?.tenant_id) {
      try {
        const { data: result, error: rpcErr } = await supabase
          .rpc("apply_agreement_credit_on_cancel" as any, { _agreement_id: id });
        if (rpcErr) throw rpcErr;
        logger.info(MODULE, "previous_agreement_credit", { agreement_id: id, result });
        if (result && (result as any).status === "applied") {
          logAction({
