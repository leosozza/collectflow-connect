-- Update get_carteira_grouped to include data_pagamento
CREATE OR REPLACE FUNCTION public.get_carteira_grouped(_tenant_id uuid, _page integer DEFAULT 1, _page_size integer DEFAULT 50, _search text DEFAULT NULL::text, _credor text DEFAULT NULL::text, _date_from date DEFAULT NULL::date, _date_to date DEFAULT NULL::date, _status_cobranca_ids uuid[] DEFAULT NULL::uuid[], _tipo_devedor_ids uuid[] DEFAULT NULL::uuid[], _tipo_divida_ids uuid[] DEFAULT NULL::uuid[], _score_min integer DEFAULT NULL::integer, _score_max integer DEFAULT NULL::integer, _debtor_profiles text[] DEFAULT NULL::text[], _sort_field text DEFAULT 'created_at'::text, _sort_dir text DEFAULT 'desc'::text, _operator_id uuid DEFAULT NULL::uuid, _sem_acordo boolean DEFAULT false, _cadastro_de date DEFAULT NULL::date, _cadastro_ate date DEFAULT NULL::date, _sem_whatsapp boolean DEFAULT false, _primeira_parcela_de date DEFAULT NULL::date, _primeira_parcela_ate date DEFAULT NULL::date)
 RETURNS TABLE(representative_id uuid, cpf text, nome_completo text, credor text, phone text, email text, data_vencimento date, valor_total numeric, valor_pago_total numeric, parcelas_count bigint, propensity_score integer, status_cobranca_id uuid, status text, debtor_profile text, operator_id uuid, external_id text, all_ids uuid[], total_count bigint, data_pagamento date)
 LANGUAGE plpgsql
...
DECLARE
  ...
  _search_digits text;
  _search_norm text;
  _has_digits boolean := false;
BEGIN
  _offset := (_page - 1) * _page_size;

  IF _search IS NOT NULL AND _search <> '' THEN
    _search_digits := regexp_replace(_search, '\D', '', 'g');
    IF _search_digits = '' THEN
      _search_digits := NULL;
    END IF;
    _search_norm := unaccent(lower(trim(_search)));
    _has_digits := _search_digits IS NOT NULL;
  END IF;
  ...
  WITH filtered AS (
    SELECT
      ...
      CASE
        ...
        WHEN unaccent(lower(c.nome_completo)) = _search_norm THEN 4
        WHEN unaccent(lower(c.nome_completo)) ILIKE _search_norm || '%' THEN 3
        WHEN unaccent(lower(c.nome_completo)) ILIKE '%' || _search_norm || '%' THEN 2
        ELSE 1
      END AS match_score
    FROM clients c
  ...
  grouped AS (
    SELECT
      ...
      SUM(COALESCE(f.valor_total, 0))::numeric AS valor_total,
      ...
      MAX(f.data_quitacao)::date AS data_pagamento
    FROM filtered f
