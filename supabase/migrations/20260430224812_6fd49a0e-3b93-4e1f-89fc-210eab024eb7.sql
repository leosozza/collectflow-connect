...
DECLARE
  ...
  -- NOVA REGRA: Quebra começa a contar a partir de D+3 (provisória reversível)
  -- e se torna definitiva quando o acordo é cancelado OU passa de D+10
...
BEGIN
  ...
  -- Estágio 1 (Provisória, reversível): acordo vivo com vencimento entre D-10 e D-3 não pago
  -- Estágio 2 (Definitiva): acordo cancelled OU vencimento <= D-10
  ...
  WHERE vc.cpf <> ''
    AND NOT EXISTS (SELECT 1 FROM agreements ag WHERE ag.tenant_id=_tenant AND ag.cpf_cnpj=vc.cpf AND ag.created_at::date >= CURRENT_DATE - 1);
