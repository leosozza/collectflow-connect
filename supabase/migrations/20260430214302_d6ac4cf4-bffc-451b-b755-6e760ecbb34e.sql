...
BEGIN
  ...
  -- =========================================================================
  -- QUEBRA (NOVA REGRA): por parcela
  --   * Acordo cancelled (auto_expired/manual)
  --   * Parcela vence no mês alvo
  --   * Vencimento <= data de cancelamento (a.updated_at) -> parcelas futuras pós-quebra ficam de fora
  --   * Aquela parcela específica NÃO foi paga (manual_payments / negociarie_cobrancas / portal_payments)
  -- =========================================================================
  ...
  FROM (
    ...
    WHERE a.tenant_id = _tenant
      ...
      AND COALESCE(a.entrada_date, a.first_due_date) <= a.updated_at::date
      AND (_no_op_filter OR a.created_by = _user_id OR a.created_by = ANY(COALESCE(_user_ids,'{}'::uuid[])))
      AND NOT EXISTS (
        SELECT 1 FROM manual_payments mp
        WHERE mp.tenant_id = _tenant AND mp.agreement_id = a.id
          AND mp.status IN ('confirmed','approved')
          AND (mp.installment_key = 'entrada' OR mp.installment_number = 0)
      )
      AND NOT EXISTS (
        SELECT 1 FROM negociarie_cobrancas nc
        WHERE nc.tenant_id = _tenant AND nc.agreement_id = a.id
          AND nc.status = 'pago'
          AND nc.installment_key = a.id::text || ':0'
      )
      AND NOT EXISTS (
        SELECT 1 FROM portal_payments pp
        WHERE pp.tenant_id = _tenant AND pp.agreement_id = a.id
          AND pp.status = 'paid'
          AND pp.installment_key = 'entrada'
      )
    UNION ALL
    -- parcelas 1..N
    SELECT COALESCE((a.custom_installment_values->>cast(gs.i as text))::numeric, a.new_installment_value) AS parcela_valor
    ...
    WHERE a.tenant_id = _tenant
      ...
      AND (a.first_due_date + ((gs.i - 1) * interval '1 month'))::date >= _month_start
      AND (a.first_due_date + ((gs.i - 1) * interval '1 month'))::date <= _month_end
      AND (a.first_due_date + ((gs.i - 1) * interval '1 month'))::date <= a.updated_at::date
      AND (_no_op_filter OR a.created_by = _user_id OR a.created_by = ANY(COALESCE(_user_ids,'{}'::uuid[])))
      AND NOT EXISTS (
        SELECT 1 FROM manual_payments mp
        WHERE mp.tenant_id = _tenant AND mp.agreement_id = a.id
          AND mp.status IN ('confirmed','approved')
          AND (mp.installment_key = cast(gs.i as text) OR mp.installment_number = gs.i)
      )
      AND NOT EXISTS (
        SELECT 1 FROM negociarie_cobrancas nc
        WHERE nc.tenant_id = _tenant AND nc.agreement_id = a.id
          AND nc.status = 'pago'
          AND nc.installment_key = a.id::text || ':' || gs.i::text
      )
      AND NOT EXISTS (
        SELECT 1 FROM portal_payments pp
        WHERE pp.tenant_id = _tenant AND pp.agreement_id = a.id
          AND pp.status = 'paid'
          AND pp.installment_key = cast(gs.i as text)
      )
  ) sub;
  ...
  FROM (
    ...
    WHERE a.tenant_id = _tenant
      ...
      AND a.cancellation_type IN ('auto_expired','manual')
      AND a.entrada_value > 0
      AND COALESCE(a.entrada_date, a.first_due_date) >= _prev_month_start
      AND COALESCE(a.entrada_date, a.first_due_date) <= _prev_month_end
      AND COALESCE(a.entrada_date, a.first_due_date) <= a.updated_at::date
      AND (_no_op_filter OR a.created_by = _user_id OR a.created_by = ANY(COALESCE(_user_ids,'{}'::uuid[])))
      AND NOT EXISTS (
        SELECT 1 FROM manual_payments mp
  ...
  -- =========================================================================
  -- PENDENTE (NOVA REGRA): por parcela
  --   * Acordo vivo (pending/approved/overdue)
  --   * Parcela vence no mês alvo
  --   * Aquela parcela específica NÃO foi paga
  --   * Sem subtração agregada do recebido
  -- =========================================================================
