...
-- 1) RLS de campaign_celebration_views: usar profile_id, não auth.uid()
...
CREATE POLICY "Operators view own celebration views"
  ...
  TO authenticated
  USING (
    operator_id = public.get_my_profile_id()
...
-- 2) get_dashboard_vencimentos: aceitar manual_payments confirmed/approved
--    e fazer fallback por installment_key além de installment_number.
    ...
    CASE
      WHEN EXISTS (
        ...
        WHERE mp.agreement_id = a.id
          AND (
            mp.installment_key = a.id::text || ':0'
