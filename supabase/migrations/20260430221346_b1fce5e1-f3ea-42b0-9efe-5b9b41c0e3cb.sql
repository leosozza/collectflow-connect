...
BEGIN
  ...
  -- QUEBRA (per-installment) — inalterado
  SELECT COALESCE(SUM(parcela_valor), 0) INTO _quebra
  FROM (
    SELECT COALESCE((a.custom_installment_values->>'entrada')::numeric, a.entrada_value) AS parcela_valor
    FROM agreements a
    WHERE a.tenant_id = _tenant AND a.status = 'cancelled' AND a.cancellation_type IN ('auto_expired','manual')
  ...
  FROM visited_cpfs v
  WHERE v.cpf <> '' AND length(v.cpf) >= 11
