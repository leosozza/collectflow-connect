-- Phase 5.3: Consolidated client status via SSOT
-- Creates get_client_consolidated_status() and rewires get_carteira_grouped to use it

CREATE OR REPLACE FUNCTION public.get_client_consolidated_status(
  _tenant_id uuid,
  _cpf text,
  _credor text,
  _atraso_quebra_dias int DEFAULT 15
) RETURNS text
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  _cpf_norm text := regexp_replace(COALESCE(_cpf, ''), '\D', '', 'g');
  _today date := (now() AT TIME ZONE 'America/Sao_Paulo')::date;
  _has_active_agreement bool := false;
  _has_cancelled_agreement bool := false;
  _worst_agreement_state text := NULL; -- 'quitado'|'acordo_vigente'|'acordo_atrasado'|'quebra_acordo'
  _has_open_debt bool := false;
  _has_overdue_debt bool := false;
BEGIN
  -- Loop active agreements; derive each agreement's state from SSOT
  FOR _has_active_agreement IN
    SELECT TRUE
    FROM agreements a
    WHERE a.tenant_id = _tenant_id
      AND regexp_replace(a.client_cpf, '\D', '', 'g') = _cpf_norm
      AND a.credor = _credor
      AND a.status IN ('pending', 'approved')
  LOOP
    -- handled below in single CTE
    EXIT;
  END LOOP;

  -- Aggregate per-agreement states using SSOT
  WITH ag AS (
    SELECT a.id, a.status AS agreement_status
    FROM agreements a
    WHERE a.tenant_id = _tenant_id
      AND regexp_replace(a.client_cpf, '\D', '', 'g') = _cpf_norm
      AND a.credor = _credor
      AND a.status IN ('pending', 'approved', 'cancelled', 'completed')
  ),
  state_per_agreement AS (
    SELECT
      ag.id,
      ag.agreement_status,
      COUNT(*) FILTER (WHERE NOT ai.cancelled) AS total_active,
      COUNT(*) FILTER (WHERE NOT ai.cancelled AND ai.paid) AS paid_active,
      COUNT(*) FILTER (WHERE NOT ai.cancelled AND NOT ai.paid AND ai.due_date < _today) AS overdue_active,
      MAX(_today - ai.due_date) FILTER (WHERE NOT ai.cancelled AND NOT ai.paid AND ai.due_date < _today) AS max_overdue_days
    FROM ag
    LEFT JOIN agreement_installments ai ON ai.agreement_id = ag.id
    GROUP BY ag.id, ag.agreement_status
  ),
  classified AS (
    SELECT
      CASE
        WHEN agreement_status = 'cancelled' THEN 'quebra_acordo'
        WHEN agreement_status = 'completed' THEN 'quitado'
        WHEN total_active = 0 THEN NULL
        WHEN paid_active = total_active THEN 'quitado'
        WHEN overdue_active > 0 AND COALESCE(max_overdue_days, 0) > _atraso_quebra_dias THEN 'quebra_acordo'
        WHEN overdue_active > 0 THEN 'acordo_atrasado'
        ELSE 'acordo_vigente'
      END AS state
    FROM state_per_agreement
  )
  SELECT
    bool_or(state IS NOT NULL) AS has_any,
    bool_or(state = 'quebra_acordo') AS has_quebra,
    -- pick worst by hierarchy
    CASE
      WHEN bool_or(state = 'quebra_acordo') THEN 'quebra_acordo'
      WHEN bool_or(state = 'acordo_atrasado') THEN 'acordo_atrasado'
      WHEN bool_or(state = 'acordo_vigente') THEN 'acordo_vigente'
      WHEN bool_or(state = 'quitado') THEN 'quitado'
      ELSE NULL
    END
  INTO _has_active_agreement, _has_cancelled_agreement, _worst_agreement_state
  FROM classified;

  -- If we have an agreement-derived state that's not quitado, it dominates
  IF _worst_agreement_state IS NOT NULL AND _worst_agreement_state <> 'quitado' THEN
    RETURN _worst_agreement_state;
  END IF;

  -- No agreements OR all agreements quitado — check raw debt in clients
  SELECT
    bool_or(c.status NOT IN ('pago')),
    bool_or(c.status NOT IN ('pago') AND c.data_vencimento < _today)
  INTO _has_open_debt, _has_overdue_debt
  FROM clients c
  WHERE c.tenant_id = _tenant_id
    AND regexp_replace(c.cpf, '\D', '', 'g') = _cpf_norm
    AND c.credor = _credor;

  IF NOT COALESCE(_has_open_debt, false) THEN
    -- All clients rows are 'pago' OR no rows: if had agreement it's quitado, else em_dia
    RETURN COALESCE(_worst_agreement_state, 'em_dia');
  END IF;

  IF _has_overdue_debt THEN
    RETURN 'inadimplente';
  END IF;

  RETURN 'em_dia';
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_client_consolidated_status(uuid, text, text, int) TO authenticated, service_role;

-- Helper: map canonical status to legacy frontend status (zero contract change)
CREATE OR REPLACE FUNCTION public.map_canonical_to_legacy_status(_canonical text)
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE _canonical
    WHEN 'quitado'         THEN 'pago'
    WHEN 'acordo_vigente'  THEN 'em_acordo'
    WHEN 'acordo_atrasado' THEN 'em_acordo'
    WHEN 'quebra_acordo'   THEN 'quebrado'
    WHEN 'inadimplente'    THEN 'vencido'
    WHEN 'em_dia'          THEN 'pendente'
    ELSE 'pendente'
  END;
$$;

GRANT EXECUTE ON FUNCTION public.map_canonical_to_legacy_status(text) TO authenticated, service_role;

-- Rewire get_carteira_grouped to use SSOT for the group-level status
CREATE OR REPLACE FUNCTION public.get_carteira_grouped(
  _tenant_id uuid,
  _page integer DEFAULT 1,
  _page_size integer DEFAULT 50,
  _search text DEFAULT NULL::text,
  _credor text DEFAULT NULL::text,
  _date_from date DEFAULT NULL::date,
  _date_to date DEFAULT NULL::date,
  _status_cobranca_ids uuid[] DEFAULT NULL::uuid[],
  _tipo_devedor_ids uuid[] DEFAULT NULL::uuid[],
  _tipo_divida_ids uuid[] DEFAULT NULL::uuid[],
  _score_min integer DEFAULT NULL::integer,
  _score_max integer DEFAULT NULL::integer,
  _debtor_profiles text[] DEFAULT NULL::text[],
  _sort_field text DEFAULT 'created_at'::text,
  _sort_dir text DEFAULT 'desc'::text,
  _operator_id uuid DEFAULT NULL::uuid,
  _sem_acordo boolean DEFAULT false,
  _cadastro_de date DEFAULT NULL::date,
  _cadastro_ate date DEFAULT NULL::date,
  _sem_whatsapp boolean DEFAULT false,
  _primeira_parcela_de date DEFAULT NULL::date,
  _primeira_parcela_ate date DEFAULT NULL::date
)
RETURNS TABLE(
  representative_id uuid, cpf text, nome_completo text, credor text,
  phone text, email text, data_vencimento date, valor_total numeric,
  valor_pago_total numeric, parcelas_count bigint, propensity_score integer,
  status_cobranca_id uuid, status text, debtor_profile text,
  operator_id uuid, external_id text, all_ids uuid[], total_count bigint
)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE
  _offset int;
  _search_digits text;
  _search_norm text;
  _has_digits bool;
BEGIN
  _offset := GREATEST(_page - 1, 0) * _page_size;
  _search_digits := CASE WHEN _search IS NULL OR _search = '' THEN NULL
                         ELSE regexp_replace(_search, '\D', '', 'g') END;
  _search_norm   := CASE WHEN _search IS NULL OR _search = '' THEN NULL
                         ELSE unaccent(lower(_search)) END;
  _has_digits    := _search_digits IS NOT NULL AND length(_search_digits) > 0;

  RETURN QUERY
  WITH filtered AS (
    SELECT
      c.id, c.cpf, c.nome_completo, c.credor, c.phone, c.phone2, c.phone3,
      c.email, c.data_vencimento, c.valor_parcela, c.valor_pago,
      c.propensity_score, c.status_cobranca_id, c.status,
      c.debtor_profile, c.operator_id, c.external_id, c.created_at,
      c.tipo_devedor_id, c.tipo_divida_id,
      CASE
        WHEN _search IS NULL OR _search = '' THEN 0
        WHEN _has_digits AND regexp_replace(c.cpf, '\D', '', 'g') = _search_digits THEN 100
        WHEN _has_digits AND regexp_replace(c.cpf, '\D', '', 'g') LIKE _search_digits || '%' THEN 80
        WHEN NOT _has_digits AND unaccent(lower(c.nome_completo)) ILIKE _search_norm || '%' THEN 70
        WHEN NOT _has_digits AND unaccent(lower(c.nome_completo)) ILIKE '%' || _search_norm || '%' THEN 50
        ELSE 10
      END AS match_score
    FROM clients c
    WHERE c.tenant_id = _tenant_id
      AND (
        _search IS NULL OR _search = ''
        OR (
          NOT _has_digits
          AND unaccent(lower(c.nome_completo)) ILIKE '%' || _search_norm || '%'
        )
        OR (
          _has_digits AND (
            regexp_replace(c.cpf, '\D', '', 'g') LIKE '%' || _search_digits || '%'
            OR regexp_replace(COALESCE(c.phone,''), '\D', '', 'g') LIKE '%' || _search_digits || '%'
            OR regexp_replace(COALESCE(c.phone2,''), '\D', '', 'g') LIKE '%' || _search_digits || '%'
            OR regexp_replace(COALESCE(c.phone3,''), '\D', '', 'g') LIKE '%' || _search_digits || '%'
            OR c.email ILIKE '%' || _search || '%'
          )
        )
      )
      AND (_credor IS NULL OR _credor = '' OR _credor = 'todos' OR c.credor = _credor)
      AND (_date_from IS NULL OR c.data_vencimento >= _date_from)
      AND (_date_to IS NULL OR c.data_vencimento <= _date_to)
      AND (_status_cobranca_ids IS NULL OR c.status_cobranca_id = ANY(_status_cobranca_ids))
      AND (_tipo_devedor_ids IS NULL OR c.tipo_devedor_id = ANY(_tipo_devedor_ids))
      AND (_tipo_divida_ids IS NULL OR c.tipo_divida_id = ANY(_tipo_divida_ids))
      AND (_debtor_profiles IS NULL OR c.debtor_profile::text = ANY(_debtor_profiles))
      AND (_operator_id IS NULL OR c.operator_id = _operator_id)
      AND (_cadastro_de IS NULL OR c.created_at::date >= _cadastro_de)
      AND (_cadastro_ate IS NULL OR c.created_at::date <= _cadastro_ate)
      AND (_score_min IS NULL OR COALESCE(c.propensity_score, 0) >= _score_min)
      AND (_score_max IS NULL OR COALESCE(c.propensity_score, 0) <= _score_max)
  ),
  grouped AS (
    SELECT
      (array_agg(f.id ORDER BY f.data_vencimento ASC))[1] AS representative_id,
      f.cpf,
      (array_agg(f.nome_completo ORDER BY f.data_vencimento ASC))[1] AS nome_completo,
      f.credor,
      (array_agg(f.phone ORDER BY f.data_vencimento ASC))[1] AS phone,
      (array_agg(f.email ORDER BY f.data_vencimento ASC))[1] AS email,
      MIN(f.data_vencimento)::date AS data_vencimento,
      SUM(COALESCE(f.valor_parcela, 0))::numeric AS valor_total,
      SUM(COALESCE(f.valor_pago, 0))::numeric AS valor_pago_total,
      COUNT(*)::bigint AS parcelas_count,
      MAX(COALESCE(f.propensity_score, 0))::int AS propensity_score,
      (array_agg(f.status_cobranca_id ORDER BY
        CASE f.status
          WHEN 'vencido' THEN 1
          WHEN 'em_acordo' THEN 2
          WHEN 'pendente' THEN 3
          WHEN 'quebrado' THEN 4
          WHEN 'pago' THEN 5
          ELSE 6
        END, f.data_vencimento ASC
      ) FILTER (WHERE f.status_cobranca_id IS NOT NULL))[1] AS status_cobranca_id,
      -- SSOT-canonical status mapped to legacy contract
      public.map_canonical_to_legacy_status(
        public.get_client_consolidated_status(_tenant_id, f.cpf, f.credor)
      ) AS status,
      (array_agg(f.debtor_profile ORDER BY f.data_vencimento ASC) FILTER (WHERE f.debtor_profile IS NOT NULL))[1]::text AS debtor_profile,
      (array_agg(f.operator_id ORDER BY f.data_vencimento ASC) FILTER (WHERE f.operator_id IS NOT NULL))[1] AS operator_id,
      (array_agg(f.external_id ORDER BY f.data_vencimento ASC) FILTER (WHERE f.external_id IS NOT NULL))[1] AS external_id,
      array_agg(f.id) AS all_ids,
      MIN(f.created_at) AS first_created_at,
      MAX(f.match_score) AS match_score
    FROM filtered f
    GROUP BY f.cpf, f.credor
    HAVING (
      (_primeira_parcela_de IS NULL AND _primeira_parcela_ate IS NULL)
      OR (
        MIN(f.data_vencimento) FILTER (WHERE f.status <> 'pago') IS NOT NULL
        AND (_primeira_parcela_de  IS NULL OR MIN(f.data_vencimento) FILTER (WHERE f.status <> 'pago') >= _primeira_parcela_de)
        AND (_primeira_parcela_ate IS NULL OR MIN(f.data_vencimento) FILTER (WHERE f.status <> 'pago') <= _primeira_parcela_ate)
      )
    )
  ),
  sem_acordo_filter AS (
    SELECT g.*
    FROM grouped g
    WHERE NOT _sem_acordo OR NOT EXISTS (
      SELECT 1 FROM agreements a
      WHERE a.tenant_id = _tenant_id
        AND a.status IN ('pending', 'approved')
        AND replace(replace(a.client_cpf, '.', ''), '-', '') = replace(replace(g.cpf, '.', ''), '-', '')
        AND a.credor = g.credor
    )
  ),
  sem_whatsapp_filter AS (
    SELECT s.*
    FROM sem_acordo_filter s
    WHERE NOT _sem_whatsapp OR (
      NOT EXISTS (
        SELECT 1 FROM whatsapp_campaign_recipients r
        JOIN clients cl ON cl.id = r.representative_client_id
        WHERE cl.tenant_id = _tenant_id
          AND r.sent_at IS NOT NULL
          AND r.status IN ('sent','delivered','read')
          AND replace(replace(cl.cpf, '.', ''), '-', '') = replace(replace(s.cpf, '.', ''), '-', '')
          AND cl.credor = s.credor
      )
      AND NOT EXISTS (
        SELECT 1 FROM chat_messages cm
        JOIN conversations cv ON cv.id = cm.conversation_id
        JOIN clients cl ON cl.id = cv.client_id
        WHERE cm.tenant_id = _tenant_id
          AND cm.direction = 'outbound'
          AND replace(replace(cl.cpf, '.', ''), '-', '') = replace(replace(s.cpf, '.', ''), '-', '')
          AND cl.credor = s.credor
      )
    )
  ),
  counted AS (
    SELECT *, COUNT(*) OVER() AS total_count
    FROM sem_whatsapp_filter
  )
  SELECT
    c.representative_id, c.cpf, c.nome_completo, c.credor,
    c.phone, c.email, c.data_vencimento, c.valor_total,
    c.valor_pago_total, c.parcelas_count, c.propensity_score,
    c.status_cobranca_id, c.status, c.debtor_profile,
    c.operator_id, c.external_id, c.all_ids, c.total_count
  FROM counted c
  ORDER BY
    CASE WHEN _search IS NOT NULL AND _search <> '' THEN c.match_score END DESC NULLS LAST,
    CASE WHEN _sort_field = 'created_at' AND _sort_dir = 'desc' THEN c.first_created_at END DESC,
    CASE WHEN _sort_field = 'created_at' AND _sort_dir = 'asc' THEN c.first_created_at END ASC,
    CASE WHEN _sort_field = 'data_vencimento' AND _sort_dir = 'desc' THEN c.data_vencimento END DESC,
    CASE WHEN _sort_field = 'data_vencimento' AND _sort_dir = 'asc' THEN c.data_vencimento END ASC,
    CASE WHEN _sort_field = 'nome_completo' AND _sort_dir = 'asc' THEN c.nome_completo END ASC,
    CASE WHEN _sort_field = 'nome_completo' AND _sort_dir = 'desc' THEN c.nome_completo END DESC,
    c.first_created_at DESC
  OFFSET _offset
  LIMIT _page_size;
END;
$function$;