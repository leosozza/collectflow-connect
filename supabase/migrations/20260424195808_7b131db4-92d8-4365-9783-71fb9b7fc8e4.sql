...
BEGIN
  ...
  WHERE a.tenant_id = _tenant AND a.created_at::date >= _month_start AND a.created_at::date <= _month_end
    AND a.status NOT IN ('cancelled', 'rejected') AND (_user_id IS NULL OR a.created_by = _user_id);
  ...
  SELECT COALESCE(SUM(COALESCE((ce.metadata->>'valor_pago')::numeric, (ce.metadata->>'amount_paid')::numeric, 0)), 0) INTO _recebido
  FROM client_events ce
  WHERE ce.tenant_id = _tenant AND ce.event_type IN ('payment_confirmed', 'manual_payment_confirmed')
    AND ce.created_at::date >= _month_start AND ce.created_at::date <= _month_end
    AND (_user_id IS NULL OR EXISTS (SELECT 1 FROM agreements a WHERE a.id = (ce.metadata->>'agreement_id')::uuid AND a.created_by = _user_id));

  SELECT COALESCE(SUM(COALESCE((ce.metadata->>'valor_pago')::numeric, (ce.metadata->>'amount_paid')::numeric, 0)), 0) INTO _recebido_mes_ant
  FROM client_events ce
  WHERE ce.tenant_id = _tenant AND ce.event_type IN ('payment_confirmed', 'manual_payment_confirmed')
    AND ce.created_at::date >= _prev_month_start AND ce.created_at::date <= _prev_month_end
    AND (_user_id IS NULL OR EXISTS (SELECT 1 FROM agreements a WHERE a.id = (ce.metadata->>'agreement_id')::uuid AND a.created_by = _user_id));
  ...
  FROM (
    SELECT COALESCE((a.custom_installment_values->>'entrada')::numeric, a.entrada_value) AS parcela_valor
    FROM agreements a
    WHERE a.tenant_id = _tenant AND a.status = 'cancelled' AND a.cancellation_type = 'auto_expired' AND a.entrada_value > 0
      AND COALESCE(a.entrada_date, a.first_due_date) >= _month_start AND COALESCE(a.entrada_date, a.first_due_date) <= _month_end
      AND (_user_id IS NULL OR a.created_by = _user_id)
    UNION ALL
    SELECT COALESCE((a.custom_installment_values->>cast(gs.i as text))::numeric, a.new_installment_value) AS parcela_valor
    FROM agreements a CROSS JOIN LATERAL generate_series(1, a.new_installments) AS gs(i)
    WHERE a.tenant_id = _tenant AND a.status = 'cancelled' AND a.cancellation_type = 'auto_expired'
      AND (a.first_due_date + ((gs.i - 1) * interval '1 month'))::date >= _month_start
      AND (a.first_due_date + ((gs.i - 1) * interval '1 month'))::date <= _month_end
      AND (_user_id IS NULL OR a.created_by = _user_id)
  ) sub;
  ...
  WHERE tenant_id = _tenant AND created_at::date >= _month_start AND created_at::date <= _month_end
    AND status NOT IN ('cancelled', 'rejected') AND (_user_id IS NULL OR created_by = _user_id);
  ...
  WITH visited_cpfs AS (
    ...
    WHERE ual.tenant_id = _tenant
      ...
      AND (_user_id IS NULL OR ual.user_id = _user_id)
  ),
  agreed_yesterday AS (
