-- 1. Add avatar columns to conversations
ALTER TABLE public.conversations
  ADD COLUMN IF NOT EXISTS remote_avatar_url TEXT NULL,
  ADD COLUMN IF NOT EXISTS remote_avatar_fetched_at TIMESTAMPTZ NULL;

-- 2. Recreate get_visible_conversations to include remote_avatar_url
DROP FUNCTION IF EXISTS public.get_visible_conversations(uuid, integer, integer, text, uuid, uuid, boolean, text, text);
...
CREATE OR REPLACE FUNCTION public.get_visible_conversations(
 ...
 RETURNS TABLE(
   ...
   remote_avatar_fetched_at timestamp with time zone,
   status text,
   assigned_to uuid,
