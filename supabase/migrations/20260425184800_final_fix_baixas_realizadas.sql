...
BEGIN
  ...
  WITH unified AS (
    ...
    SELECT
      ...
      -- Converte o profile.id gravado em mp.requested_by para auth.user_id
    ...
    SELECT
      ...
      COALESCE((pp.payment_data->>'interest')::numeric, (pp.payment_data->>'juros')::numeric, (COALESCE(a.interest_amount, 0) / (1 + COALESCE(a.new_installments, 0)))::numeric) AS juros,
      COALESCE((pp.payment_data->>'penalty')::numeric, (pp.payment_data->>'multa')::numeric, (COALESCE(a.penalty_amount, 0) / (1 + COALESCE(a.new_installments, 0)))::numeric) AS multa,
      COALESCE((pp.payment_data->>'fees')::numeric, (pp.payment_data->>'honorarios')::numeric, (COALESCE(a.fees_amount, 0) / (1 + COALESCE(a.new_installments, 0)))::numeric) AS honorarios,
      COALESCE((pp.payment_data->>'discount')::numeric, (pp.payment_data->>'desconto')::numeric, (COALESCE(a.discount_amount, 0) / (1 + COALESCE(a.new_installments, 0)))::numeric) AS desconto,
