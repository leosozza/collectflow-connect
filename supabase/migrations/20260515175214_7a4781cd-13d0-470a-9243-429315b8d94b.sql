...
BEGIN
  ...
  -- 1) COLCHÃO: acordos vigentes (NÃO cancelados/rejeitados), 1ª parcela/entrada dentro do mês.
  --    Inclui overdue. Fica fixo no mês; só sai se for cancelado.
  ...
  FROM (
    ...
    WHERE a.tenant_id = _tenant AND a.status NOT IN ('cancelled', 'rejected')
      AND a.created_at::date < _month_start AND a.entrada_value > 0
    ...
    WHERE a.tenant_id = _tenant AND a.status NOT IN ('cancelled', 'rejected') AND a.created_at::date < _month_start
  ...
  -- 2) PROVISIONADO (Acordos Fechados no Mês): acumulador histórico puro por created_at, não desce com cancelamentos.
  SELECT COALESCE(SUM(
  ...
  -- RECEBIDO (verdade = baixas realizadas)
  _recebido :=
  ...
  -- 4) QUEBRA: somente cancelados auto_expired (sem manual = erro de operador), sem pagamento, vencimento dentro do mês.
  ...
  FROM (
    ...
    WHERE a.tenant_id = _tenant
      ...
      AND a.status = 'cancelled' AND a.cancellation_type = 'auto_expired'
      AND (_no_op_filter OR a.created_by = _user_id OR a.created_by = ANY(COALESCE(_user_ids,'{}'::uuid[])))
  ...
  -- 3) PENDENTES: tudo que tem para entrar no mês e ainda não foi pago. Acordos em atraso continuam; só sai se cancelado.
  ...
  WHERE tenant_id=_tenant AND created_at::date BETWEEN _prev_month_start AND _prev_month_last AND status NOT IN ('cancelled','rejected')
    AND (_no_op_filter OR created_by=_user_id OR created_by=ANY(COALESCE(_user_ids,'{}'::uuid[])));
