-- Migração: Ajustar agrupamento por nome e unicidade de perfis
-- Objetivo: Separar clientes com mesmo CPF mas nomes diferentes e evitar duplicatas em importações.

-- 1. Permitir que existam perfis diferentes para o mesmo CPF se o nome for diferente
-- Isso resolve o caso do CPF 000.000.000-00 (Francielda vs Luci)
ALTER TABLE public.client_profiles DROP CONSTRAINT IF EXISTS client_profiles_tenant_id_cpf_key;
DROP INDEX IF EXISTS client_profiles_tenant_id_cpf_key;
CREATE UNIQUE INDEX IF NOT EXISTS client_profiles_tenant_id_cpf_nome_key ON public.client_profiles (tenant_id, cpf, nome_completo);

-- 2. Atualizar o RPC get_carteira_grouped para agrupar por NOME também
...
BEGIN
  ...
  grouped AS (
    SELECT
      ...
      f.cpf,
      f.nome_completo, -- Agrupando agora também pelo NOME
      f.credor,
    ...
    FROM filtered f
    GROUP BY f.cpf, f.credor, f.nome_completo -- ADICIONADO NOME_COMPLETO
    HAVING (
