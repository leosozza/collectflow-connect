...
BEGIN
  ...
  FOR i IN 1.._max_pages LOOP
    ...
    INTO v_page_processed, v_page_updated, v_page_max_id;

    IF v_page_max_id IS NOT NULL THEN
      v_last_id := v_page_max_id;
    END IF;
...
GRANT EXECUTE ON FUNCTION public.sync_clients_status_loop(uuid, uuid, int, int) TO postgres, authenticated, anon, service_role;
