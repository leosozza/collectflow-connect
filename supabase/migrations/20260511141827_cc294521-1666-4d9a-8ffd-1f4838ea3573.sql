    ...
    CASE
      WHEN EXISTS (
        ...
        WHERE mp.agreement_id = a.id
          AND (
            mp.installment_key = 'entrada'
            OR mp.installment_key = a.id::text || ':entrada'
            OR mp.installment_key = a.id::text || ':0'
            OR mp.installment_number = 0
          )
      ...
      WHEN EXISTS (
        ...
        WHERE nc.agreement_id = a.id
          AND (
            nc.installment_key = a.id::text || ':entrada'
            OR nc.installment_key = a.id::text || ':0'
  ...
  SELECT
    ...
    CASE
      WHEN EXISTS (
        ...
        WHERE mp.agreement_id = a.id
          AND (
            mp.installment_key = (i + 1)::text
            OR mp.installment_key = a.id::text || ':' || (i + 1)::text
            OR mp.installment_number = (i + 1)
      ...
      WHEN EXISTS (
        ...
        WHERE nc.agreement_id = a.id
          AND (
            nc.installment_key = a.id::text || ':' || (i + 1)::text
            OR nc.installment_key = (i + 1)::text
          )
          AND nc.status = 'pago'
      ) THEN 'paid'
      WHEN a.status = 'overdue' THEN 'overdue'
