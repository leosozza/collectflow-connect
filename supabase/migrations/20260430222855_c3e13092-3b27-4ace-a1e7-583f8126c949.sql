...
DECLARE
  ...
  _quebra_ceiling date := CURRENT_DATE - 10; -- vencimentos <= a esta data e não pagos viram QUEBRA
...
BEGIN
  ...
  -- QUEBRA (per-installment) — NOVA REGRA: tolerância de 10 dias
  -- Soma parcelas do mês não pagas quando:
  --   (a) acordo cancelado (auto/manual) E vencimento <= updated_at; OU
  --   (b) vencimento <= hoje - 10 dias (independente do status), pegando casos onde o auto-cancel ainda não rodou
  ...
  -- QUEBRA mês anterior — mesma regra (snapshot atual)
  ...
  -- PENDENTE — janela: GREATEST(month_start, hoje-3) .. month_end
  ...
  -- Pendente do mês anterior: snapshot do mês inteiro (comparativo)
  ...
  FROM visited_cpfs v
  WHERE v.cpf <> '' AND NOT EXISTS (SELECT 1 FROM agreed_yesterday g WHERE g.cpf = v.cpf);

  RETURN QUERY SELECT _projetado, _negociado, _negociado_mes, _recebido, _quebra, _pendente,
                      _dia, _mes, _acionados_ontem, _dia_ant, _mes_ant,
