WITH ranked_accounts AS (
  SELECT id, row_number() OVER (PARTITION BY provider ORDER BY updated_at DESC, created_at DESC) AS rn
  FROM public.platform_billing_accounts WHERE is_active = true
)
UPDATE public.platform_billing_accounts pba SET is_active = false
FROM ranked_accounts ranked WHERE pba.id = ranked.id AND ranked.rn > 1;

CREATE UNIQUE INDEX IF NOT EXISTS uq_platform_billing_provider_active
  ON public.platform_billing_accounts (provider) WHERE is_active = true;
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_platform_billing_active_subscription
  ON public.platform_billing_subscriptions (tenant_id, platform_account_id) WHERE status = 'ACTIVE';
CREATE INDEX IF NOT EXISTS idx_platform_billing_customers_tenant ON public.platform_billing_customers (tenant_id);
CREATE INDEX IF NOT EXISTS idx_platform_billing_subscriptions_tenant ON public.platform_billing_subscriptions (tenant_id);
CREATE INDEX IF NOT EXISTS idx_platform_billing_subscriptions_status ON public.platform_billing_subscriptions (status);
...
CREATE POLICY "Super admins can manage platform billing customers"
  ON public.platform_billing_customers FOR ALL TO authenticated
...
CREATE POLICY "Super admins can manage platform billing subscriptions"
  ON public.platform_billing_subscriptions FOR ALL TO authenticated
  USING (public.is_super_admin(auth.uid())) WITH CHECK (public.is_super_admin(auth.uid()));

DROP TRIGGER IF EXISTS trg_platform_billing_customers_updated_at ON public.platform_billing_customers;
CREATE TRIGGER trg_platform_billing_customers_updated_at BEFORE UPDATE ON public.platform_billing_customers
...
CREATE TRIGGER trg_platform_billing_subscriptions_updated_at BEFORE UPDATE ON public.platform_billing_subscriptions
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();