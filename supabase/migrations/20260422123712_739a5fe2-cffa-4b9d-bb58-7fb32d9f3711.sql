CREATE OR REPLACE FUNCTION public.get_carteira_grouped(_tenant_id uuid, _page integer DEFAULT 1, _page_size integer DEFAULT 50, _search text DEFAULT NULL::text, _credor text DEFAULT NULL::text, _date_from date DEFAULT NULL::date, _date_to date DEFAULT NULL::date, _status_cobranca_ids uuid[] DEFAULT NULL::uuid[], _tipo_devedor_ids uuid[] DEFAULT NULL::uuid[], _tipo_divida_ids uuid[] DEFAULT NULL::uuid[], _score_min integer DEFAULT NULL::integer, _score_max integer DEFAULT NULL::integer, _debtor_profiles text[] DEFAULT NULL::text[], _sort_field text DEFAULT 'created_at'::text, _sort_dir text DEFAULT 'desc'::text, _operator_id uuid DEFAULT NULL::uuid, _sem_acordo boolean DEFAULT false, _cadastro_de date DEFAULT NULL::date, _cadastro_ate date DEFAULT NULL::date, _sem_whatsapp boolean DEFAULT false)
...
BEGIN
  ...
  WITH filtered AS (
    ...
    WHERE c.tenant_id = _tenant_id
      AND (_search IS NULL OR _search = '' OR _search_words IS NULL OR
           (unaccent(c.nome_completo) ILIKE ALL (SELECT '%' || unaccent(unnest(_search_words)) || '%')) OR
           (_search_digits IS NOT NULL AND regexp_replace(c.cpf, '\D', '', 'g') LIKE '%' || _search_digits || '%') OR
           (_search_digits IS NOT NULL AND (
             regexp_replace(COALESCE(c.phone,''), '\D', '', 'g') LIKE '%' || _search_digits || '%' OR
             regexp_replace(COALESCE(c.phone2,''), '\D', '', 'g') LIKE '%' || _search_digits || '%' OR
             regexp_replace(COALESCE(c.phone3,''), '\D', '', 'g') LIKE '%' || _search_digits || '%'
           )) OR
  ...
  sem_whatsapp_filter AS (
    ...
    WHERE NOT _sem_whatsapp OR (
      -- No effective campaign dispatch
      NOT EXISTS (
      ...
      -- And no outbound chat message (atendimento/régua/workflow/API)
      AND NOT EXISTS (
