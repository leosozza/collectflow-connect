...
RETURNS TABLE(
  source text, payment_id uuid, agreement_id uuid, client_cpf text, client_name text,
  credor text, installment_number integer, total_installments integer, installment_key text,
  valor_original numeric, juros numeric, multa numeric, honorarios numeric, desconto numeric,
  valor_pago numeric, payment_date date, payment_method text, local_pagamento text, operator_id uuid
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH unified AS (
    ...
    SELECT
      ...
      COALESCE(
        NULLIF(mp.interest_amount, 0),
        (COALESCE(a.interest_amount, 0) / GREATEST(
          1,
          (SELECT COUNT(*)::int FROM jsonb_object_keys(COALESCE(a.custom_installment_values, '{}'::jsonb)) k WHERE k LIKE 'entrada%')
          + COALESCE(a.new_installments, 0)
        ))::numeric
      ...
      COALESCE(
        NULLIF(mp.penalty_amount, 0),
        (COALESCE(a.penalty_amount, 0) / GREATEST(
          1,
          (SELECT COUNT(*)::int FROM jsonb_object_keys(COALESCE(a.custom_installment_values, '{}'::jsonb)) k WHERE k LIKE 'entrada%')
          + COALESCE(a.new_installments, 0)
        ))::numeric
      ...
      COALESCE(
        NULLIF(mp.fees_amount, 0),
        (COALESCE(a.fees_amount, 0) / GREATEST(
          1,
          (SELECT COUNT(*)::int FROM jsonb_object_keys(COALESCE(a.custom_installment_values, '{}'::jsonb)) k WHERE k LIKE 'entrada%')
          + COALESCE(a.new_installments, 0)
        ))::numeric
      )::numeric AS honorarios,
      COALESCE(
        NULLIF(mp.discount_amount, 0),
        (COALESCE(a.discount_amount, 0) / GREATEST(
          1,
          (SELECT COUNT(*)::int FROM jsonb_object_keys(COALESCE(a.custom_installment_values, '{}'::jsonb)) k WHERE k LIKE 'entrada%')
          + COALESCE(a.new_installments, 0)
        ))::numeric
    ...
    SELECT
      ...
      COALESCE(
        NULLIF((pp.payment_data->>'interest')::numeric, 0),
        (COALESCE(a.interest_amount, 0) / GREATEST(
          1,
          (SELECT COUNT(*)::int FROM jsonb_object_keys(COALESCE(a.custom_installment_values, '{}'::jsonb)) k WHERE k LIKE 'entrada%')
          + COALESCE(a.new_installments, 0)
      ...
      COALESCE(
        NULLIF((pp.payment_data->>'penalty')::numeric, 0),
        (COALESCE(a.penalty_amount, 0) / GREATEST(
          1,
          (SELECT COUNT(*)::int FROM jsonb_object_keys(COALESCE(a.custom_installment_values, '{}'::jsonb)) k WHERE k LIKE 'entrada%')
          + COALESCE(a.new_installments, 0)
      ...
      COALESCE(
        NULLIF((pp.payment_data->>'fees')::numeric, 0),
        (COALESCE(a.fees_amount, 0) / GREATEST(
          1,
          (SELECT COUNT(*)::int FROM jsonb_object_keys(COALESCE(a.custom_installment_values, '{}'::jsonb)) k WHERE k LIKE 'entrada%')
          + COALESCE(a.new_installments, 0)
      ...
      COALESCE(
        NULLIF((pp.payment_data->>'discount')::numeric, 0),
        (COALESCE(a.discount_amount, 0) / GREATEST(
          1,
          (SELECT COUNT(*)::int FROM jsonb_object_keys(COALESCE(a.custom_installment_values, '{}'::jsonb)) k WHERE k LIKE 'entrada%')
          + COALESCE(a.new_installments, 0)
