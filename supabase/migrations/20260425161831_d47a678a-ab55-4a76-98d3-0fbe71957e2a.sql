-- FASE 1: Atualizar get_dashboard_stats para incluir negociarie_cobrancas e status 'approved' em manual_payments
...
BEGIN
  ...
  -- TOTAL RECEBIDO: manual_payments(confirmed/approved) + portal_payments(paid) + negociarie_cobrancas(pago)
  _recebido :=
...
-- FASE 3: Nova RPC get_baixas_realizadas
...
BEGIN
  ...
  WITH unified AS (
    ...
    SELECT
      ...
      NULL::text,
      pp.amount,
      COALESCE((pp.payment_data->>'interest')::numeric, 0),
      COALESCE((pp.payment_data->>'penalty')::numeric, 0),
      COALESCE((pp.payment_data->>'fees')::numeric, 0),
      pp.amount,
      pp.updated_at::date,
      pp.payment_method,
      'cobradora'::text
