...
BEGIN
  ...
  -- Total Projetado: sum all virtual installments (entrada + mensalidades) whose due date falls in the month
  SELECT COALESCE(SUM(parcela_valor), 0) INTO _projetado
  FROM (
    -- Entrada (parcela 0) with entrada_date in the target month
    ...
    -- Monthly installments: first_due_date + (i-1) months for i = 1..new_installments
  ...
  -- Total Primeira Parcela: for agreements created in the month, sum only the first payment
  SELECT COALESCE(SUM(
