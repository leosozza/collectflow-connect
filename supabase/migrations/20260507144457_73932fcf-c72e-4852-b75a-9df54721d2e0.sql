CREATE OR REPLACE FUNCTION public.get_baixas_realizadas(_date_from date DEFAULT NULL::date, _date_to date DEFAULT NULL::date, _credor text DEFAULT NULL::text, _local text DEFAULT NULL::text, _payment_method text DEFAULT NULL::text, _operator_id uuid DEFAULT NULL::uuid)
...
BEGIN
  ...
  WITH unified AS (
    ...
    SELECT
      ...
      (COALESCE(a.interest_amount, 0) / NULLIF(1 + COALESCE(a.new_installments, 0),0))::numeric AS juros,
      (COALESCE(a.penalty_amount, 0) / NULLIF(1 + COALESCE(a.new_installments, 0),0))::numeric AS multa,
      (COALESCE(a.fees_amount, 0) / NULLIF(1 + COALESCE(a.new_installments, 0),0))::numeric AS honorarios,
      (COALESCE(a.discount_amount, 0) / NULLIF(1 + COALESCE(a.new_installments, 0),0))::numeric AS desconto,
      nc.valor_pago,
      nc.data_pagamento,
      nc.tipo,
      'cobradora'::text,
