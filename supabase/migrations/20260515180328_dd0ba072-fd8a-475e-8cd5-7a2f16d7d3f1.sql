...
BEGIN
  ...
  -- 1) COLCHÃO: foto do mês.
  -- Mês corrente: cancelamentos do PRÓPRIO mês continuam no colchão (status NOT IN cancelados OU updated_at >= início do mês).
  -- Outros meses: apenas vigentes.
  ...
  FROM (
    ...
    WHERE a.tenant_id = _tenant
      AND (
        ...
        OR (_is_current_month AND a.updated_at::date >= _month_start)
      )
  ...
  -- 2) PROVISIONADO (Acordos Fechados no Mês): acumulador histórico puro.
  SELECT COALESCE(SUM(
  ...
  -- 4) CANCELADOS (antes "Quebra"): apenas auto_expired sem pagamento.
