-- Expandir fallback de instância nas RPCs de visibilidade:
-- Antes: visível só se assigned_to IS NULL E client_id IS NULL E operador vinculado à instância.
-- Agora: visível se assigned_to IS NULL E operador vinculado à instância
--        E (client_id IS NULL OU clients.operator_id IS NULL OU clients.operator_id = _profile_id).
-- Mudança puramente aditiva: nada que era visível deixa de ser; apenas revela conversas órfãs
-- na instância dedicada do operador, preservando carteira atribuída de outros operadores.

-- ============================================================
-- Overload 1: get_visible_conversations (sem _disposition_filter)
-- ============================================================
...
RETURNS TABLE(id uuid, tenant_id uuid, instance_id uuid, remote_phone text, remote_name text, remote_avatar_url text, remote_avatar_fetched_at timestamp with time zone, status text, assigned_to uuid, last_message_at timestamp with time zone, unread_count integer, client_id uuid, client_name text, last_message_content text, last_message_type text, last_message_direction text, created_at timestamp with time zone, updated_at timestamp with time zone, sla_deadline_at timestamp with time zone, total_count bigint)
LANGUAGE plpgsql
...
BEGIN
  ...
  WITH visible AS (
    SELECT
      ...
      c.remote_avatar_url AS conv_remote_avatar_url,
      c.remote_avatar_fetched_at AS conv_remote_avatar_fetched_at,
  ...
  SELECT
    ...
    cc.conv_remote_name,
    cc.conv_remote_avatar_url,
    cc.conv_remote_avatar_fetched_at,
    cc.conv_status,
    cc.conv_assigned_to,
...
-- ============================================================
-- Overload 2: get_visible_conversations (com _disposition_filter)
-- ============================================================
CREATE OR REPLACE FUNCTION public.get_visible_conversations(
  _tenant_id uuid,
  _page integer DEFAULT 1,
...
BEGIN
  ...
  RETURN QUERY
  WITH visible AS (
    SELECT c.status AS conv_status, c.unread_count AS conv_unread
    FROM public.conversations c
    WHERE c.tenant_id = _tenant_id
      AND (
        _is_admin
  ...
  SELECT
    COUNT(*) FILTER (WHERE v.conv_status = 'open')::bigint,
    COUNT(*) FILTER (WHERE v.conv_status = 'waiting')::bigint,
    COUNT(*) FILTER (WHERE v.conv_status = 'closed')::bigint,
    COUNT(*) FILTER (WHERE v.conv_unread > 0)::bigint
  FROM visible v;
END;
$function$;