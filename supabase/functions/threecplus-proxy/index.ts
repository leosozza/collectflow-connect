  ...
  try {
    ...
    switch (action) {
      ...
      case 'create_work_break_group_interval': {
        ...
        url = buildUrl(baseUrl, `work_break_group/${body.group_id}/intervals`, authParam);
        method = 'POST';
        const typeMap: Record<string, number> = { productive: 1, unproductive: 2, nr17: 3 };
        const returnMap: Record<string, number> = { flexible: 1, automatic: 2, request: 3 };
      ...
      case 'update_work_break_group_interval': {
        ...
        method = 'PUT';
        const uTypeMap: Record<string, number> = { productive: 1, unproductive: 2, nr17: 3 };
        const uReturnMap: Record<string, number> = { flexible: 1, automatic: 2, request: 3 };
        const updateBody: Record<string, any> = {};
        if (body.name) updateBody.name = body.name;
        if (body.max_time != null && body.max_time !== '') updateBody.limit = Number(body.max_time);
        if (body.daily_limit != null && body.daily_limit !== '') updateBody.maximum_daily_time = Number(body.daily_limit);
        if (body.color) updateBody.color = body.color;
        if (body.classification) updateBody.type = (uTypeMap[body.classification] ?? Number(body.classification)) || 1;
        if (body.return_type) updateBody.return_type = (uReturnMap[body.return_type] ?? Number(body.return_type)) || 1;
      ...
      // ── Webhook Management ── (removed: 3CPlus does not expose REST webhook endpoints)
