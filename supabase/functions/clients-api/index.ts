...
function validateClientRecord(record: Record<string, unknown>): ValidationResult {
  ...
  if (record.data_vencimento && typeof record.data_vencimento === "string") {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(record.data_vencimento as string)) {
      errors.push("data_vencimento: formato inválido (esperado YYYY-MM-DD)");
    }
  } else {
    errors.push("data_vencimento: obrigatório (formato YYYY-MM-DD)");
  }
...
Deno.serve(async (req: Request) => {
  ...
  if (segments[0] === "clients" && segments[1] === "bulk" && method === "POST") {
    const body = await req.json() as { records: Record<string, unknown>[]; upsert?: boolean; upsert_key?: string };
    ...
    const upsertKey = body.upsert_key ?? "external_id";
    ...
    if (validRows.length > 0) {
      ...
      if (doUpsert) {
        ...
        // Approximate: treat all as either insert or update
